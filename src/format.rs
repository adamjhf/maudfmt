...
pub fn preprocess_source_for_ignore(source: &str) -> Result<(String, Vec<&str>)> {
    ...
    while i < lines.len() {
        ...
        if let Some((_, comment_part)) = line.split_once("//") {
            ...
            if comment_trimmed.starts_with("maudfmt-ignore") && i + 1 < lines.len() {
                ...
                processed_lines.push("\"__MAUDFMT_IGNORED_PLACEHOLDER__\"");
...
pub fn reinsert_ignored_lines_in_source(
    ...
    for line in lines {
        if line.trim() == "\"__MAUDFMT_IGNORED_PLACEHOLDER__\"" && ignore_index < ignore_info.len()
        {
